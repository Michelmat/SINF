...
if ( !function_exists( 'rara_readable_move_comment_field_to_bottom' ) ) :
    /**
     * Hook to move comment text field to the bottom
     * 
     * @link http://www.wpbeginner.com/wp-tutorials/how-to-move-comment-text-field-to-bottom-in-wordpress-4-4/  
     */
    function rara_readable_move_comment_field_to_bottom( $fields ) {
        $comment_field = $fields['comment'];
        unset( $fields['comment'] );
        $fields['comment'] = '<p class="comment-form-comment"><label for="comment">'. esc_html__( 'Comment', 'rara-readable' ) .'</label><textarea id="comment" name="comment" cols="40" rows="8" required="required" placeholder="'. esc_attr__( 'Comment', 'rara-readable' ) .'"></textarea></p>';
        return $fields;
    }
endif;
add_filter( 'comment_form_fields', 'rara_readable_move_comment_field_to_bottom' );
